...
export default function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit} aria-describedby="form-note" className="w-full min-w-0 max-w-full rounded-[2rem] border border-white/10 bg-[#151519] p-6 sm:p-8">
      ...
      <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><p id="form-note" className="max-w-xs text-xs leading-5 text-white/45">Форма работает в демонстрационном режиме. Отправка наружу отключена.</p><button type="submit" className="inline-flex min-h-12 items-center justify-center rounded-full bg-[#F7F6F2] px-6 py-3 text-sm font-semibold text-[#0C0C0E] transition-transform hover:-translate-y-0.5 focus:outline-none">Отправить форму</button></div>
    </form>
  );
}
